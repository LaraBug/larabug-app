      ...
      <ul class="divide-y divide-gray-200">
        <li v-for="issue in issues.data" :key="issue.id">
          <inertia-link :href="route('panel.issues.show', issue.id)" class="flex items-center px-6 py-4 space-x-6 hover:bg-gray-100">
            <div class="w-2 h-2 rounded-full" :class="{'bg-red-600' : issue.status === 'OPEN', 'bg-gray-300' : !issue.status === 'CLOSED' }"></div>

            <div class="flex-1">
              <p class="font-medium text-bold">{{ issue.exception }}</p>
              <p class="text-sm text-gray-600">{{ issue.exception_count }} exceptions &centerdot; Last exception {{ issue.human_date }}</p>
            </div>
